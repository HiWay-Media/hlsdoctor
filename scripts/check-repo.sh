...
for f in README.md CLAUDE.md CONTRIBUTING.md LICENSE BACKLOG.md ROADMAP.md CHANGELOG.md deploy/nomad/hlsdoctor.nomad.hcl deploy/streams.example.txt deploy/nomad/hlsdoctor-docker.nomad.hcl Dockerfile .dockerignore; do [ -f "$f" ] || fail "$f is missing"; done
...
grep -q 'ghcr.io/hiway-media/hlsdoctor' deploy/nomad/hlsdoctor-docker.nomad.hcl || fail "the docker Nomad job must run the GHCR image"
grep -q 'type *= *"batch"' deploy/nomad/hlsdoctor-docker.nomad.hcl || fail "the docker Nomad job must be a periodic batch job"
